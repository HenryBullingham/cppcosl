...
/// <summary>
/// Calls count_to_n and then count_down_n
/// </summary>
/// <param name=""></param>
/// <returns></returns>
co_declare(work_in_sequence)
{
    cppcosl::co_local<cppcosl::co_handle> h1, h2;

    co_begin();

    std::cout << "Yielding for nested coroutines" << std::endl;

    yield_call_args(count_to_n, 10);

    yield_call_args(count_down_n, 10);

    std::cout << "Done yielding" << std::endl;

    yield_break();
}

/// <summary>
/// Calls cppcosl::lib::foreach twice in series on a vector of pair<int, char> and prints the elements separately
/// </summary>
/// <param name=""></param>
/// <returns></returns>
co_declare(sequential_foreach)
{
...
co_declare_args(co_merge_sort, int* list, int size)
{
    ...
    if (size > 1)
    {
        yield_call_args(co_merge_sort, list, size / 2);
        yield_call_args(co_merge_sort, list + size / 2, size - size / 2);
...
int main(int argc, char** argv)
{
    ...
    auto handle = cppcosl::co_start(work_in_sequence);

    cppcosl::co_wait(handle);

    handle = cppcosl::co_start(sequential_foreach);
